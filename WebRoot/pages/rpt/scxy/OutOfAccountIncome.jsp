...
function querytitle(){
	...
	jQuery.ajax({
 		url: '<%=contextPath%>/rpt/scxy/OutOfAccountIncome!querytitle.action',
 	    type: "POST",
 	    data: {
 			"monthId": monthId
 		},
 		...
 		success:function(data){
	    	 ...
	    	 if(data.list.length>0){
	 			    var title = data.list[0];
	 			        document.getElementById("part1").innerHTML = title.AVG_VALUE_ADD_RATE;
	 ...
	 function querytable(){
	 	var monthId = jQuery('#monthId').val();
	 	jQuery.ajax({
	 		url: '<%=contextPath%>/rpt/scxy/OutOfAccountIncome!queryOutTable.action',
	 	    type: "POST",
	 	    data: {
	 			"monthId": monthId
	 		},
	...
	function queryChart(){
		var monthId = jQuery('#monthId').val();
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/scxy/OutOfAccountIncome!queryOutLineChart.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
	...
	function barChart(data, chartName, chartDivId) {
		...
		for (var i = 0; i < bar1List.length; i++) {
			bar1Data.push((bar1List[i].AVG_VALUE_ADD_RATE*1).toFixed(0));
	...
	function getSubList(obj,regionId){
		...
		var monthId = jQuery('#monthId').val();
	 	jQuery.ajax({
	 		url: '<%=contextPath%>/rpt/scxy/OutOfAccountIncome!getLastInfo.action',
	 	    type: "POST",
	 	    data: {
	 			"monthId": monthId,
	 			"areaNo":regionId
		...
		<div class="row" style="height: 30px;margin-top: 10px;">
			<div class="col-xs-12 col-sm-12 col-md-12" style="padding: 0px;">
				<table style="border-collapse: collapse;height: 28px;padding: 0px;margin: auto;width: 100%;">
					...
					<td style="background-color: #BFBFBF;text-align: left;padding-left: 10px;font-weight: bold;font-size: 14px;">不含2I2C移动出账收入同比变动</td>
					</tr>
				</table>
			</div>
		</div>
		<div class="row" style="text-align: left;margin-left:5px;font-weight: bold;font-size: 14px;padding-top:1%;">
		      <span id="part"></span>月，全省不含2I2C移动出账收入同比增加<span id="part1">---</span>亿元，同比增幅<span id="part2">---</span>%<br>
		 </div>
