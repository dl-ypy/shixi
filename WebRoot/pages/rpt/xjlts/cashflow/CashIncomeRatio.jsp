...
<script type="text/javascript">
	...
	function table1() {
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/xjlts/cashflow/CashIncomeRatio!getCashIncomeRatioTable.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
	...
	function part1TableClick(AREA_NO, AREA_DESC){
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/xjlts/cashflow/CashIncomeRatio!getCashIncomeRatioLine.action',
			type:"POST",
		    data: {
				"monthId": monthId,
				"areaNo": AREA_NO
			...
			success:function(data) {
				lineChart(data,"现金收入比变动趋势","part1LineDiv");
			}
		});
	}
	function getBarChart1(){
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/xjlts/cashflow/CashIncomeRatio!getCashIncomeRatioBarChart.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
			...
			success:function(data) {
				if(data.barList != null && data.barList.length > 0){
					barChart(data,"现金收入比排名","part1BarDiv");
				}else{
	...
	function lineChart(data, chartName, chartDivId) {
		...
		for (var i = 0; i < line1List.length; i++) {
			line1Data.push(line1List[i].KPI_04);
		}
		var line2List = data.thisYearLine;
		var line2Data = [];
		for (var i = 0; i < line2List.length; i++) {
			line2Data.push(line2List[i].KPI_04);
		}
	...
	function barChart(data, chartName, chartDivId) {
		...
		for (var i = 0; i < bar1List.length; i++) {
			...
			bar2Data.push((bar1List[i].KPI_11*1).toFixed(0));
			lineData.push((bar1List[i].KPI_12*100).toFixed(2));
