	...
	<head>
	    ...
	    <script type="text/javascript" src="../../../resources/frame/echarts-3.8.4/map/henan.js"></script>
	  	<script type="text/javascript">
	  		...
	  		function table() {
	  			jQuery.ajax({
	  				url:'<%=contextPath%>/rpt/xjlts/yyqkfx/AnalysisChanges!getTableList.action',
	  				type:'post',
	  				data:{
	  					"monthId":monthId,
	  					"areaNo":areaNo
	  		...
	  		function tableClick(level_id){
	  			jQuery.ajax({
					url: '<%=contextPath%>/rpt/xjlts/yyqkfx/AnalysisChanges!getWarnText.action',
					type:"POST",
				    data: {
				   /*level_id=orgID  */ 	
						"monthId": monthId,
			...
			function getMapChart(){
				jQuery.ajax({
					url: '<%=contextPath%>/rpt/xjlts/yyqkfx/AnalysisChanges!getMapChart.action',
					type:"POST",
				    data: {
						"monthId": monthId,
						"areaNo":areaNo
		...
		function mapChart(data,chartDivId) {
			...
			for (var i = 0; i < mapList.length; i++) {
				var subJsonData = {"name": mapList[i].AREA_DESC, "value": (mapList[i].KPI_07 * 100).toFixed(2)};
		...
		function tableDetail(areaNo_mapClick){
			...
			jQuery.ajax({
  				url:'<%=contextPath%>/rpt/xjlts/yyqkfx/AnalysisChanges!getTableDetail.action',
  				type:'post',
  				data:{
  					"monthId":monthId,
				...
				<div class="col-xs-12 col-sm-12 col-md-12" style="padding: 0px;">
					<table style="border-collapse: collapse;height: 28px;padding: 0px;margin: auto;width: 100%;">
						<tr>
							...
							<td style="background-color: #BFBFBF;text-align: left;padding-left: 10px;font-weight: bold;font-size: 14px;">营业欠款变动分析</td>
						</tr>
					</table>
				</div>
			</div>
								...
								<tr>
									<td>
										<div style="float: left; margin-top: 10px;font-size: 14px;">
											<span id="KPI">--</span>营业欠款变动绝对值超5%营业厅合计<span id="KPI_01">--</span>个,其中前三
											<span id="KPI_02">--</span>,分别<span id="KPI_03">--</span>个.
										</div>
									</td>	
								</tr>
							</tbody>
