...
<link rel="stylesheet" type="text/css" href="../struts/pure/msg/navigation.css" />
...
<body id="<%=SystemConfigProperty.FRAME_LAYOUT%>">
	<div class="pure-frame-body">
		<div id="pure-frame-top" style="height: 50px;">
			<div id="pure-frame-top-logo">
				<div style="font-family: 微软雅黑, 宋体; font-weight: bold; font-size: 26px; margin-left: 170px; margin-top: 10px; color: #656565; width: 300px">
					河南联通价值管理系统
				</div>
			</div>
			...
			<ul id="pure-sys-tool">
				...
				<%if(SystemConfigProperty.IS_DISPLAY_ONLINE){%><li><a id="sys-tool-online">在线()</a></li><%}%>
		...
		<div id="nav-menu-bar">
			<%if(SystemConfigProperty.IS_DISPLAY_SYSTIME){%>
				<div id="nav-menu-bar-datetime">
			...
			<s:if test="resources==null||resources.size()==0">
				<span style="color:red;height:39px;line-height:39px;">您当前没有该系统的任何菜单权限，请联系管理员！</span>
				<iframe name="noIframe" scrolling="auto" frameborder="no" border="0" width="100%" min-height="300px"></iframe>
			</s:if>
			<div style="clear: both;"> </div>
		</div>
	</div>
	<!-- 公告右下角提示 -->
	<%if(SystemConfigProperty.IS_DISPLAY_POPMESSAGE){%>
	<div id="pop" class="sys-window" style="display:none;right:3px;bottom:3px;width:300px;">
		<div id="popHead" class="sys-window-handler" >
		  <div class="sys-window-title">提示</div>
		   <ul class="sys-window-handler-btn">
		        <li id="popClose" class="pop-window-btn-close-btn"  title="关闭"><a style="width:16px;">&nbsp;</a></li>
		   </ul>
	    </div>
	   <div id="popContent" class="sys-window-container" style="overflow: auto;height:130px;"></div>
	</div>
	<%} %>
	...
	var userinfo = function(){
		if(typeof(frame) == "undefined") { 
			$j('iframe[name="noIframe"]').attr('src', Bonc.toFullPath('/pure/UserInfo.action'));
		}
		else{
					...
					if (o==null||!(o instanceof Resources)) {
						...
						if(StringUtils.isNotBlank(res.getUrl())){
							...
							JSONObject json = new JSONObject()
									...
									.element("url", url)
							.element("leaf", res.getResourcesType().getResourcesTypeId()==7L?true:res.getChildren()==null||res.getChildren().size()==0)
	...
	$j('#modify-password').click(function(){
		...
		if(typeof(frame) == "undefined") { 
			$j('iframe[name="noIframe"]').attr('src', Bonc.toFullPath('/pure/ModifyPwd'));
		}else{
	...
	<%if(SystemConfigProperty.IS_DISPLAY_ANNOUNCEMENT){%>
	...
	<%if(SystemConfigProperty.IS_DISPLAY_ONLINE){%>
	//在线人数
	$j('#sys-tool-online').click(function(){
		new Bonc.Window({
			id:'sys-tool-online-window',
			resizable: true,
			dragable:false,
			height:350,
			width:610,
	...
	var reshOnLine=function(){
		$j.ajax({
	     url:Bonc.toFullPath('/pure/LogEnter!totalOnLine.action'),
	     async: false,
	     global:false,
	     data:{},
	     success:function(data){
	      	jQuery('#sys-tool-online').text('在线('+data.totalOnLine+')');
	     }
	 });
	}
	 $j(document).ready(function(){
		   reshOnLine();
		   setInterval(reshOnLine,50000);
	   });
	 <%}%>
    ...
    //右下角公告提示
    <%if(SystemConfigProperty.IS_DISPLAY_POPMESSAGE){%>
    var autoHideTimer,mouseoutTimer,
    reshShowPop=function(){    
			   jQuery.ajax({
					url:Bonc.toFullPath('/pure/Post!remind.action'),
		         	async: false,
		         	 type: "post",  
		         	 global:false,
		         	success:function(data){
		         		 $j('#popContent').html(data);
		         		 if($j('#popContent').children('#data').val()>0){
		         			$('#pop').slideDown(1000);
