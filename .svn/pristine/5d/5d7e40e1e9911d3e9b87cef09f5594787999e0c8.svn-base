...
<script type="text/javascript">
	...
	function table1() {
		/* 首行ajax */
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/zhwg/ProfitSchedule!getProfitScheduleFirstLine.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
			...
			success:function(data) {
				...
				if(data.firstLineList != null && data.firstLineList.length > 0){
					...
					var firstLineDiv = "<div align='left' style='font-weight:bold; font-size:14px; padding-top:1%;'>"+month+"月，全省"+firstLineList.KPI_07+"个综合网格，利润进度在100%~105%之间的有"+firstLineList.KPI_04+"个，105%~110%之间的有"+
					firstLineList.KPI_05+"个，110%以上的有"+firstLineList.KPI_06+"个，95%~100%之间的有"+firstLineList.KPI_03+"个、90%~95%之间的有"+firstLineList.KPI_02+"个、90%以下的有"+firstLineList.KPI_01+"个。</div>";
		...
		/* 表格ajax */
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/zhwg/ProfitSchedule!getProfitScheduleTable.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
	...
	function getBarChart1(areaNo){
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/zhwg/ProfitSchedule!getProfitScheduleBar.action',
			type:"POST",
		    data: {
				"monthId": monthId,
				"areaNo":areaNo
			...
			cache:false,
			success:function(data) {
				barChart(data,data.barList[0]!=null?data.barList[0].AREA_DESC+"利润进度分布":"暂无数据","part1BarDiv");
			}
		});
	}
	...
	function barChart(data, chartName, chartDivId) {
		...
		var option = {
			    ...
			    xAxis: {
			    	type: 'category',
			        data: ['90%以下','90%~95%','95%~100%','100%~105%','105%~110%','110%以上'],
			        axisLabel:{
			        	interval:0
			        }
			    },
  ...
  <body>
    <div class="container" style="width: 98%;margin-top: 5px;height: 0px;">
	 ...
	 <div class="container" style="width: 98%;margin-top: 2%;" id="homeframe-container">
	 	<div class="row" style="height: 30px;margin-top: 10px;">
			<div class="col-xs-12 col-sm-12 col-md-12" style="padding: 0px;">
				<table style="border-collapse: collapse;height: 28px;padding: 0px;margin: auto;width: 100%;">
					...
					<td style="background-color: #BFBFBF;text-align: left;padding-left: 10px;font-weight: bold;font-size: 14px;">利润进度分布情况</td>
					</tr>
				</table>
			</div>
		</div>
		...
		<div class="row">
			<div class="col-xs-6 col-sm-6 col-md-6" id="part1TableDiv"></div>
			<div class="col-xs-6 col-sm-6 col-md-6" id="part1BarDiv" style="height: 380px; margin-top: 25px; padding:10px;"></div>
		</div>
	</div>
  </body>
</html>
