...
function table() {
	jQuery.ajax({
		url: '<%=contextPath%>/rpt/ztdb/BszxdbSywcqk!table.action',
		type:"POST",
	    data: {
			"monthId": monthId,
			"orgId": areaNo
...
function loadOrgTableDrill(obj,kpiId,orgId){
	var url= '<%=contextPath%>/rpt/ztdb/BszxdbSywcqk!getOrgTable.action';
...
function tableClick(kpiId, chartName,orgId) {
	jQuery.ajax({
		url: '<%=contextPath%>/rpt/ztdb/BszxdbSywcqk!chart.action',
		type:"POST",
	    data: {
			"monthId": monthId,
			"orgId": orgId,
...
function lineChart(line1Data, line2Data, chartName, chartDivId, chartNameDivId) {
    ...
    function queryTableDrill(obj, kpiId,orgId){
    	var url= '<%=contextPath%>/rpt/ztdb/BszxdbSywcqk!table.action';
		...
		<div class="row" style="height: 30px;margin-top: 10px;">
			<div class="col-xs-12 col-sm-12 col-md-12" style="padding: 0px;">
				<table style="border-collapse: collapse;height: 28px;padding: 0px;margin: auto;width: 100%;">
					...
					<td style="background-color: #BFBFBF;text-align: left;padding-left: 10px;font-weight: bold;font-size: 14px;">损益完成情况</td>
					</tr>
				</table>
			</div>
		</div>
