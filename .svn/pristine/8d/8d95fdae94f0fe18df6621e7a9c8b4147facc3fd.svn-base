...
function table() {
	jQuery.ajax({
		url: '<%=contextPath%>/rpt/ztdb/BszxdbScxcb!table.action',
		type:"POST",
	    data: {
			"monthId": monthId,
			"orgId": areaNo
...
function tableClick(kpiId, chartName) {
	jQuery.ajax({
		url: '<%=contextPath%>/rpt/ztdb/BszxdbScxcb!chart.action',
		type:"POST",
	    data: {
			"monthId": monthId,
			"orgId": areaNo,
...
function lineChart(line1Data, line2Data, chartName, chartDivId, chartNameDivId) {
    ...
    function queryTableDrill(obj, kpiId, orgId){
    	var url= '<%=contextPath%>/rpt/ztdb/BszxdbScxcb!table.action';
 ...
 // 按组织机构下钻
    function drillTableByOrg(obj, kpiId, orgId){
    	var url= '<%=contextPath%>/rpt/ztdb/BszxdbScxcb!queryDrillTableByOrg.action';
    	var param = "monthId=" + monthId + "&kpiId=" + kpiId + "&orgId=" + orgId;
    	action({
    		td : obj,
    		url : url,
    		mode : 'drill',
		...
		<div class="row" style="height: 30px;margin-top: 10px;">
			<div class="col-xs-12 col-sm-12 col-md-12" style="padding: 0px;">
				<table style="border-collapse: collapse;height: 28px;padding: 0px;margin: auto;width: 100%;">
					...
					<td style="background-color: #BFBFBF;text-align: left;padding-left: 10px;font-weight: bold;font-size: 14px;">市场线执行成本情况</td>
					</tr>
				</table>
			</div>
		</div>
		<div class="row">
			<div class="col-xs-9 col-sm-9 col-md-9" id="tableDiv"></div>
