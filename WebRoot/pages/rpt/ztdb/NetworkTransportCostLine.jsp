...
<script type="text/javascript">
	...
	function queryCumulativeTableDrill(obj, kpiId, orgId){
	...
	function linkage1(kpiId, chartName,levelId) {
		var areaNo_link =levelId==(null || "") ? areaNo : levelId;
		jQuery.ajax({
			...
			success:function(data) {
				...
				for (var i = 0; i < line1List.length; i++) {
					line1Data1.push((line1List[i].KPI_01 * 1).toFixed(0));
				...
				for (var i = 0; i < line2List.length; i++) {
					line1Data2.push((line2List[i].KPI_01 * 1).toFixed(0));
	...
	function linkage2(kpiId, chartName,levelId) {
		var areaNo_link =levelId==(null || "") ? areaNo : levelId;
		jQuery.ajax({
	 	...
	 	jQuery.ajax({
	 		...
	 		success:function(html){
				...
				jQuery(".part2Table")[0].click();
	 			/* 项目下钻 点击事件  Kpi_Id 固定？ 或者可以给一个class标签   jQuery(".drillTd")[0].click(); */
				jQuery("#bisAccountKpiTreeRootORG_KPI_001").click();
	 		}
	 	});
	 }
	function queryAccountTableDrill(obj, kpiId,orgId){
 ...
 // 按组织机构下钻
    function drillTableByOrg1(obj, kpiId, orgId){
    	var url= '<%=basePath%>/rpt/ztdb/NetworkTransportCostLine!drillTableByOrg1.action';
    	var param = "monthId=" + monthId + "&areaNo=" + areaNo + "&kpiId=" + kpiId+ "&zsbType=" + zsbType;
    	action({
    		td : obj,
    		url : url,
    ...
    function drillTableByOrg2(obj, kpiId, orgId){
    	var url= '<%=basePath%>/rpt/ztdb/NetworkTransportCostLine!drillTableByOrg2.action';
    	var param = "monthId=" + monthId + "&areaNo=" + areaNo + "&kpiId=" + kpiId+ "&zsbType=" + zsbType;
    	action({
    		td : obj,
    		url : url,
		  ...
		  <div class="col-xs-6 col-sm-6 col-md-6" > 
		      <form class="form-inline"  style="margin: 0px;"> 
		      	 ...
		      	 <div class="form-group">
                     ...
                     <select id="ZSB_TYPE" class="form-control" style="width: 100px;height: 25px;font-size: 12px;padding: 0px;">
