    ...
    $.fn.mergeTable = function(options){
        ...
        var options = $.extend(defaults, options);
            return  this.each(function(){
                ...
                //4.同时合并多行列
                var multiToMerger = options.multiToMerger;
                ...
                for(var k=0;k<multiToMerger.length;k++)
            	{
            		setMultispan($(this),multiToMerger[k].cols,multiToMerger[k].rows,multiToMerger[k].val);
            	}
                
            	//同时合并多行多列
                function setMultispan(obj,cols,rows,val)
            	{
                	var rowStart = rows[0];
                	var rowEnd = rows[1];
                	var colStart = cols[0];
                	...
                	$(obj).find("tr").slice(rowStart,rowEnd+1).each(function(i,v){
						$(this).find("td").slice(colStart,colEnd+1).each(function(index,value){
							if(i == 0 && index == 0){
								$(this).attr("colspan",colEnd - colStart + 1);
								$(this).attr("rowspan",rowEnd - rowStart + 1);
							}else{
								$(this).remove();
							}
						});
