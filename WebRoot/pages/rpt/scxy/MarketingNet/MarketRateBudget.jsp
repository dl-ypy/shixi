...
<script type="text/javascript">
	...
	function table1() {
		/* 首行ajax */
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/scxy/MarketingNet/MarketRateBudget!getMarketRateBudgetFirstLine.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
			...
			success:function(data) {
				...
				if(data.firstLineList != null && data.firstLineList.length > 0){
					...
					var firstLineDiv = "<div align='left' style='font-weight:bold; font-size:14px; padding-top:1%;'>1"+month+"月全省不含2I2C市场毛利率"+firstLineList.KPI_05+"%，较预算目标"+str+Math.abs(firstLineList.KPI_03)+"PP</div>";
		...
		/* 前三后三ajax */
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/scxy/MarketingNet/MarketRateBudget!getMarketRateBudgetRanking.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
		...
		/* 表格ajax */
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/scxy/MarketingNet/MarketRateBudget!getMarketRateBudgetTable.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
	...
	function part1TableClick(AREA_NO, AREA_DESC){
		...
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/scxy/MarketingNet/MarketRateBudget!getMarketRateBudgetLastLine.action',
			type:"POST",
		    data: {
				"monthId": monthId,
				"areaNo": AREA_NO
			...
			success:function(data) {
				...
				if(data.lastLineList != null && data.lastLineList.length > 0){
					...
					if(lastlist.KPI_01-lastlist.KPI_06 > 0){
						var lastLine = "<div>"+lastlist.AREA_DESC+"较预算"+str+Math.abs(lastlist.KPI_03)+"pp,高于全省均值"+Math.abs(lastlist.KPI_01-lastlist.KPI_06)+"pp</div>";
						jQuery("#lastLineDiv").html(lastLine);
					}else if(lastlist.KPI_01-lastlist.KPI_06 < 0){
						var lastLine = "<div>"+lastlist.AREA_DESC+"较预算"+str+Math.abs(lastlist.KPI_03)+"pp,低于全省均值"+Math.abs(lastlist.KPI_01-lastlist.KPI_06)+"pp</div>";
						jQuery("#lastLineDiv").html(lastLine);
					}else{
						var lastLine = "<div>"+lastlist.AREA_DESC+"较预算"+str+Math.abs(lastlist.KPI_03)+"pp</div>";
						jQuery("#lastLineDiv").html(lastLine);
					}
				}else{
	...
	function getBarChart1(){
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/scxy/MarketingNet/MarketRateBudget!getMarketRateBudgetBarChart.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
	...
	function barChart(data, chartName, chartDivId) {
		...
		for (var i = 0; i < bar1List.length; i++) {
			bar0Data.push(bar1List[i].KPI_02);
	 	...
	 	<div class="row" style="height: 30px;margin-top: 10px;">
			<div class="col-xs-12 col-sm-12 col-md-12" style="padding: 0px;">
				<table style="border-collapse: collapse;height: 28px;padding: 0px;margin: auto;width: 100%;">
					...
					<td style="background-color: #BFBFBF;text-align: left;padding-left: 10px;font-weight: bold;font-size: 14px;">不含2I2C市场毛利率预算完成</td>
					</tr>
				</table>
			</div>
		</div>
