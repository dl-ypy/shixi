...
<script type="text/javascript">
	...
	function querytitle(){
		...
		jQuery.ajax({
	 		url: '<%=contextPath%>/rpt/zhwg/ProfitScheduleDB!querytitle.action',
	 	    type: "POST",
	 	    data: {
	 			"monthId": monthId,
	 			"areaNo": 'root'
	...
	function querytable(){
	 	...
	 	jQuery.ajax({
	 		url: '<%=contextPath%>/rpt/zhwg/ProfitScheduleDB!queryOutTable.action',
	 	    type: "POST",
	 	    data: {
	 			"monthId": monthId,
	 			"areaNo": areaNo
	...
	function queryChart(){
		var monthId = jQuery('#monthId').val();
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/zhwg/ProfitScheduleDB!queryOutChart.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
	 ...
	 <div class="container" style="width: 98%;margin-top: 2%;" id="homeframe-container">
			...
			<div class="col-xs-12 col-sm-12 col-md-12" style="padding: 0px;">
				<table style="border-collapse: collapse;height: 28px;padding: 0px;margin: auto;width: 100%;">
					<tr>
						...
						<td style="background-color: #BFBFBF;text-align: left;padding-left: 10px;font-weight: bold;font-size: 14px;">利润进度对标</td>
					</tr>
				</table>
			</div>
		</div>
		<div class="row" style="text-align: left;margin-left:5px;font-weight: bold;font-size: 14px;padding-top:1%;">
			<!-- 
			 ...
			 -->
			 1月，全省综合网格利润进度XX%，全省整体利润进度XX%，综合网格利润进度较整体高（低）XXpp。
		</div>
		<div class="row">
