...
<script type="text/javascript">
	...
	function table1() {
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/scxy/innovate/BusinessRate!getBusinessRateFirstLine.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
			...
			success:function(data) {
				...
				if(data.firstLineList != null && data.firstLineList.length > 0){
					...
					var firstLineDiv = "<div align='left' style='font-weight:bold; font-size:14px; padding-top:1%'>1"+month+"月全省2I2C业务毛利率"+firstLineList.KPI_01+"%，同比"+firstStr+Math.abs(firstLineList.KPI_03)+"pp</div>";
		...
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/scxy/innovate/BusinessRate!getBusinessRateRanking.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
		...
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/scxy/innovate/BusinessRate!getBusinessRateTable.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
	...
	function part1TableClick(AREA_NO, AREA_DESC){
		...
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/scxy/innovate/BusinessRate!getBusinessRateLastLine.action',
			type:"POST",
		    data: {
				"monthId": monthId,
				"areaNo": AREA_NO
			...
			success:function(data) {
				...
				if(data.lastLineList != null && data.lastLineList.length > 0){
					...
					if(lastlist.KPI_01-lastlist.KPI_05 > 0){
						var lastLine = "<div style='background:#FCD5B5; padding:20px; height:50px; width:80%;margin-left:10%;margin-top:2%;margin-bottom:3%;font-size:14px'>"+lastlist.AREA_DESC+"较上年同期"+str+Math.abs(lastlist.KPI_03)+"pp,高于全省均值"+Math.abs(lastlist.KPI_01-lastlist.KPI_05)+"pp</div>";
						jQuery("#lastLineDiv").html(lastLine);
					}else if(lastlist.KPI_01-lastlist.KPI_05 < 0){
						var lastLine = "<div style='background:#FCD5B5; padding:20px; height:50px; width:80%;margin-left:10%;margin-top:2%;margin-bottom:3%;font-size:14px'>"+lastlist.AREA_DESC+"较上年同期"+str+Math.abs(lastlist.KPI_03)+"pp,低于全省均值"+Math.abs(lastlist.KPI_01-lastlist.KPI_05)+"pp</div>";
						jQuery("#lastLineDiv").html(lastLine);
					}else{
						var lastLine = "<div style='background:#FCD5B5; padding:20px; height:50px; width:80%;margin-left:10%;margin-top:2%;margin-bottom:3%;font-size:14px'>"+lastlist.AREA_DESC+"较上年同期"+str+Math.abs(lastlist.KPI_03)+"pp</div>";
						jQuery("#lastLineDiv").html(lastLine);
					}
				}else{
	...
	function getBarChart1(){
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/scxy/innovate/BusinessRate!getBusinessRateBarChart.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
	...
	function barChart(data, chartName, chartDivId) {
		...
		for (var i = 0; i < bar1List.length; i++) {
			...
			line2Data.push(bar1List[i].KPI_02);
	  	...
	  	<div class="row" style="height: 30px;margin-top: 10px;">
			<div class="col-xs-12 col-sm-12 col-md-12" style="padding: 0px;">
				<table style="border-collapse: collapse;height: 28px;padding: 0px;margin: auto;width: 100%;">
					...
					<td style="background-color: #BFBFBF;text-align: left;padding-left: 10px;font-weight: bold;font-size: 14px;">2I2C业务毛利率同比变动</td>
					</tr>
				</table>
			</div>
		</div>
