...
<%@ tag import="com.bonc.dss.hq.fillreport.service.showreportutils.DateUtil" %>
...
<%
	...
	String defaultId = DateUtil.nextId();
	...
	if(StringUtils.isBlank(value)){
		value = (String)DateUtil.findNameValue(jspContext,request, name);
	}
	if(StringUtils.isBlank(value)){
		value = DateUtil.getFirstMonthString();
	}	
	if(StringUtils.isBlank(minYearMonth)){
		minYearMonth=DateFormatUtils.format(DateUtil.getMinMonthDate(),"yyyyMM");
	}
	if(StringUtils.isBlank(maxYearMonth)){
		maxYearMonth=DateFormatUtils.format(DateUtil.getMaxMonthDate(),"yyyyMM");
	}
	
	
	String dateStyleId = DateUtil.nextId();
%>
