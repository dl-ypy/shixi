		...
		jQuery.ajax({
			...
			success:function(data) {
				...
				if(data.firstLineList != null && data.firstLineList.length > 0){
					...
					if (firstLineList.VALUE_ADD_RATE.trim().substring(0,1) == '-') {
						str = "改善";
					} else {
						str = "恶化";
					}
			...
			success:function(data) {
				...
				if(data.lastLineList != null && data.lastLineList.length > 0){
					...
					if(lastlist.VALUE_ADD_RATE_PP > 0){
						var lastLine = "<div>"+lastlist.ORG_NAME+"较上年末"+str+"幅度"+Math.abs(lastlist.VALUE_ADD_RATE)+"%,低于全省均值"+Math.abs(lastlist.VALUE_ADD_RATE_PP)+"pp</div>";
						jQuery("#lastLineDiv").html(lastLine);
					}else if(lastlist.VALUE_ADD_RATE_PP < 0){
						var lastLine = "<div>"+lastlist.ORG_NAME+"较上年末"+str+"幅度"+Math.abs(lastlist.VALUE_ADD_RATE)+"%,高于全省均值"+Math.abs(lastlist.VALUE_ADD_RATE_PP)+"pp</div>";
						jQuery("#lastLineDiv").html(lastLine);
					}else{
						var lastLine = "<div>"+lastlist.ORG_NAME+"较上年末"+str+"幅度"+Math.abs(lastlist.VALUE_ADD_RATE)+"%</div>";
						jQuery("#lastLineDiv").html(lastLine);
					}
				}else{
