...
var projectPath = "<%=contextPath%>";
	 ...
	 function query(){
	 	...
	 	jQuery.ajax({
	 		url: '<%=contextPath%>/rpt/xjlts/checkDay/BusinessHallCheck!queryTable.action',
	 	    type: "post",
	 	    data: {
	 			"monthId": monthId, 
	 			"areaNo": areaNo
	   ...
	   function getSubList(obj, region_id){
		    var monthId = jQuery('#monthId').val();
			var url= '<%=contextPath%>/rpt/xjlts/checkDay/BusinessHallCheck!queryTableDrill.action';
...
<body>
    ...
    <%--  <input type="hidden" id="userOrgId" value='<s:property value="param.userOrgId"/>' />
     <input type="hidden" id="userOrgLevel" value='<s:property value="param.userOrgLevel"/>' />
     --%> 
	 ...
	 <div class="container" style="width: 98%;margin-top:50px;" id="homeframe-container">
			...
			<div class="col-xs-12 col-sm-12 col-md-12" style="padding: 0px;">
				<table style="border-collapse: collapse;height: 28px;padding: 0px;margin: auto;width: 100%;">
					...
					<td style="background-color: #BFBFBF;text-align: left;padding-left: 10px;font-weight: bold;font-size: 14px;">自有渠道稽核日报</td>
					</tr>
				</table>
			</div>
		</div>
