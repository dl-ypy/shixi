...
function querytitle(){
	...
	jQuery.ajax({
 		url: '<%=contextPath%>/rpt/wyxy/PortRealRate!querytitle.action',
 	    type: "POST",
 	    data: {
 			"monthId": monthId
 		},
	 ...
	 function querytable(){
	 	var monthId = jQuery('#monthId').val();
	 	jQuery.ajax({
	 		url: '<%=contextPath%>/rpt/wyxy/PortRealRate!queryOutTable.action',
	 	    type: "POST",
	 	    data: {
	 			"monthId": monthId
	 		},
	...
	function queryChart(){
		var monthId = jQuery('#monthId').val();
		jQuery.ajax({
			url: '<%=contextPath%>/rpt/wyxy/PortRealRate!queryOutLineChart.action',
			type:"POST",
		    data: {
				"monthId": monthId
			},
	...
	function getSubList(obj,regionId){
		...
		var monthId = jQuery('#monthId').val();
	 	jQuery.ajax({
	 		url: '<%=contextPath%>/rpt/wyxy/PortRealRate!getLastInfo.action',
	 	    type: "POST",
	 	    data: {
	 			"monthId": monthId,
	 			"areaNo":regionId
  ...
  <body>
	 ...
	 <div class="container" style="width: 98%;margin-top: 2%;" id="homeframe-container">
		...
		<div class="row" style="text-align: left;margin-left:5px;font-weight: bold;font-size: 14px;padding-top:1%;">
		      <span id="part"></span>月，全省宽带端口实占率为 <span id="part1">---</span>%，同比<span id="part2">---</span> PP<br>
		 </div>
